...
var CMD = function () {
    function CMD(opts) {
        ...
        var defaults = {
            ...
            save: function save(cmdData) {
                return console.warn('No save function has been set.');
            },
            load: function load() {
                return console.warn('No load function has been set.');
            },
            update: function update(cmdObj) {
                return console.warn('No update function has been set.');
            }
        };
        var options = Object.assign({}, defaults, opts);
        this.loadCommands = _commands.loadCommands;
        this.money = 0;
        this.increment = 1;
    ...
    _createClass(CMD, [{
        ...
        value: function respond() {
            this.respondFunc.apply(this, arguments);
        }
    }, {
        key: 'command',
            ...
            if (cmd.indexOf(" ") !== -1 && cmd[cmd.indexOf(" ") + 1] === undefined) {
                ...
                if (!(cmdWArgs[0] in this._commands)) {
                    ...
                    if (this._commands[cmdWArgs[0]].unlocked) {
                        var _commands$cmdWArgs$;

                        (_commands$cmdWArgs$ = this._commands[cmdWArgs[0]]).func.apply(_commands$cmdWArgs$, _toConsumableArray(cmdWArgs.slice(1)));
                    }
                }
            }
        }
        ...
        key: 'addData',
        value: function addData(amt) {
            this.data += amt;
            this.update();
        }
    }, {
        key: 'removeData',
        value: function removeData(amt) {
            this.data -= amt;
            this.update();
        }
    }, {
        ...
        key: 'removeMoney',
        value: function removeMoney(amt) {
            this.money -= amt;
            this.update();
        }
    }, {
        key: 'formatBytes',
        value: function formatBytes() {
            return _humanize2.default.filesize(this.data);
        }
    }]);
...
},{"./commands":4,"humanize":1}],4:[function(require,module,exports){
"use strict";
...
function loadCommands() {
    ...
    this._commands = {
        help: {
            func: function func(toHelp) {
                if (toHelp) {
                    if (!(toHelp in _this._commands)) {
                        _this.respond("Command not found or no help is available. Type 'help' with no arguments to see a list of commands.");
                        return;
                    }
                    var _commands$toHelp = _this._commands[toHelp];
                    var desc = _commands$toHelp.desc;
                    var usage = _commands$toHelp.usage;

                    desc = typeof desc === "function" ? desc() : desc;
                    usage = typeof usage === "function" ? usage() : usage;
                    _this.respond(toHelp + ":", desc);
                    _this.respond("To use:", toHelp + ",", usage);
                } else {
                    var availableCommands = [];
                    for (var cmdName in _this._commands) {
                        var cmd = _this._commands[cmdName];
                        if (cmd.unlocked) {
                    ...
                    var cmdList = availableCommands.join("\n\t");
                    _this.respond("########################################");
                    _this.respond('List of commands:');
                    _this.respond(cmdList);
                    _this.respond(" ");
                    _this.respond("For specific command help type 'help [command]'");
                    _this.respond("########################################");
                }
            },
        ...
        mineData: {
            func: function func() {
                _this.respond("Data mined.");
                _this.addData(_this.increment);
            },
        ...
        autoMine: {
            func: function func() {
                _this.autoIncrement = 1;
                _this.respond("Automatic mining beginning at a rate of " + _this.autoIncrement + " byte per second.");
            },
        ...
        sellData: {
            func: function func(amt) {
                if (amt) {
                    // amt = Number(amt);
                    if (_this.data >= amt && _this.data >= 100 && typeof amt !== "number") {
                        var loss = Math.floor(Math.random() * 15 + 10);
                        // console.log('Loss:', loss);
                        var transfer = Math.round(amt * (1 - loss / 100));
                        // this.money += transfer;
                        // this.data -= transfer;
                        _this.addMoney(transfer);
                        _this.removeData(transfer);
                        _this.respond(loss + "% data integrity lost in transfer. Data sold: " + amt + ". Money gained: $" + transfer + ".");
                    } else {
                        _this.respond('You must sell at least 100 data. Please make sure you have 100 data.');
                    }
                } else {
                    _this.respond("Argument needed.");
                    _this.respond(' ');
                    _this._commands.help.func('sellData');
                }
            },
        ...
        buyData: {
            func: function func(amt) {
                if (amt) {
                    var cost = amt * 2;
                    if (_this.money >= cost && typeof amt !== "number") {
                        // this.money -= cost;
                        // this.data += Number(amt);
                        _this.removeMoney(cost);
                        _this.addData(Number(amt));
                        _this.respond(amt + " data bought with $" + cost);
                    } else {
                        _this.respond("You do not have enough money.");
                    }
                } else {
                    _this.respond("Argument needed. Try: buyData [amount]");
                }
            },
        ...
        buyCommand: {
            func: function func(cmdName) {
                if (cmdName && cmdName in _this._commands) {
                    var cmd = _this._commands[cmdName];
                    if (_this.data >= cmd.price) {
                        if (!cmd.unlocked) {
                            cmd.unlocked = true;
                            _this.removeData(cmd.price);
                            _this.respond("Command unlocked: " + cmdName);
                        } else {
                            _this.respond("Command already unlocked.");
                        }
                    } else {
                        _this.respond("You don't have enough data to buy this command.");
                    }
                } else {
                    _this.respond("Command not found for purchase.");
                }
            },
            desc: function desc() {
                var cmdList = [];
                ...
                _this.respond("Purchases and unlocks a command.");
                _this.respond("Available commands:\n\t", cmdList.join("\n\t"));
            },
        ...
        load: {
            func: function func() {
                ...
                if (previousSave) {
                    ...
                    _this.data = loadData.data;
                    _this.money = loadData.money;
                    _this.increment = loadData.increment;
                    _this.autoIncrement = loadData.autoIncrement;
                    var _iteratorNormalCompletion = true;
                    var _didIteratorError = false;
                    var _iteratorError = undefined;

                    try {
                        for (var _iterator = loadData.unlocked[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                            var unlockedCMD = _step.value;

                            _this._commands[unlockedCMD].unlocked = true;
                        }
                    } catch (err) {
                    ...
                    _this.respond("Save loaded.");
                } else {
                    _this.respond("No save found.");
                }
            },
    ...
    var cmd = new _cmd.CMD({
        respond: function respond() {
            for (var i = 0; i < arguments.length; i++) {
                (0, _jquery2.default)('#responses').append('<tr class="response"><td class="response">' + arguments[i] + '</td></tr>');
            }
        },
        save: function save(cmdData) {
        ...
        update: function update(cmdObj) {
            (0, _jquery2.default)('#dataCount').html(cmdObj.formatBytes());
            (0, _jquery2.default)('#moneyCount').html('$' + cmdObj.money);
        }
    });
    (0, _jquery2.default)(document).keypress(function (e) {
        if (e.which == 13) {
            cmd.command((0, _jquery2.default)('#input').val());
            cmd.historyBufferCurrentIdx = -1;
            (0, _jquery2.default)('#cmdWindow').scrollTop((0, _jquery2.default)('#cmdWindow')[0].scrollHeight);
